...
比如我们的翻译的结果为<span style='color:red'>I am a student</span>，那么我们在decoders中，第一次的输入是<span style='color:red !important'>`<sos>`</span>，经过decoders之后得到<span style='color:red'>`I`</span>，然后将<span style='color:red'>`<sos> I`</span>一起当作输入，放入到decoders中，得到<span style='color:red'>`am`</span>
然后再将<span style='color:red'>`<sos> I am`</span> 当作输入放入到decoders中，得到结果<span style='color:red!important'>`a`</span>依次类推下去，一直到得到结束符<span style='color:red!important'>`<eos>`</span>为止。
