...
因为神经元中有dropout的存在，且从 <span style='font-weight: bold;'>公式1</span> 可以看到$$C_i$$随着$$N_i$$的二次方的变化而变化，于是在参数剪枝之后，
dropout的丢失率为如 <span style='font-weight: bold;'>公式2</span> 所示，其中$$D_o$$表示原始的丢失率，$$D_r$$表示重新训练之后的丢失率。
...
连接。这时由于梯度下降和和正则化导致的。一个拥有为0（或者输出为0）的输入的神经元，是不会对最终的损失值有贡献的，从而使得梯度在输入连接或者输出连接的地方为0。只有正则项会导致
