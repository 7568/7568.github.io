...
因为神经元中有dropout的存在，且从 <span style='font-weight: bold;color: blue;'>公式1</span> 可以看到$$C_i$$随着$$N_i$$的二次方的变化而变化，于是在参数剪枝之后，
dropout的丢失率为如 <span style='font-weight: bold;color: blue;'>公式2</span> 所示，其中$$D_o$$表示原始的丢失率，$$D_r$$表示重新训练之后的丢失率。
