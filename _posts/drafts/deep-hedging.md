...
而且，对于非限制性的策略$\delta^u \in \mathcal(H)^u$，我们（依次）定义$(H o \delta^u)_k := K_k((H o \delta^u)_0 , ... , (H o \delta^u)_{k-1} , \delta_k^u)$ 它的受约束"投影"到$\mathcal{h}_k$中。我们使用$\mathcal{H} := (H o \mathcal{H}^u) \subset \mathcal{H}^u $来表示受限制的交易策略中的对应的非空集合。
